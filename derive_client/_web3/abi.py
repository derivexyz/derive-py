...
class ContractRegistry:
    def __init__(self, w3: Web3, *, network: str):
        ...
        manifest_path = _abi_dir(network) / "contracts.json"
    ...
    def get(self, name: str) -> Contract:
        ...
        if name not in self._manifest:
            raise KeyError(f"{name!r} not in {self._network} contract manifest. Known: {sorted(self._manifest)}")
