...
@pytest.mark.asyncio
async def test_vaults_list_all(client_admin_wallet):
    with assert_api_calls(client_admin_wallet, expected=1):
        listed = await client_admin_wallet.vaults.list_all(page=1, page_size=10)
    assert isinstance(listed, VaultsResponse)
    assert all(isinstance(vault, Vault) for vault in listed.vaults)


@pytest.mark.asyncio
async def test_vaults_get(client_admin_wallet, any_vault):
    with assert_api_calls(client_admin_wallet, expected=1):
        vault = await client_admin_wallet.vaults.get(vault_subaccount_id=any_vault.protocol.subaccount_id)
    assert isinstance(vault, Vault)
    assert vault.protocol.subaccount_id == any_vault.protocol.subaccount_id


@pytest.mark.asyncio
async def test_vaults_get_defaults_to_this_subaccount(client_admin_wallet, curated_vault):
    vault_subaccount = await client_admin_wallet.fetch_subaccount(curated_vault)
    vault = await vault_subaccount.vaults.get()
...
async def test_vaults_action_history(client_admin_wallet, any_vault):
    with assert_api_calls(client_admin_wallet, expected=1):
        history = await client_admin_wallet.vaults.action_history(
...
@pytest.mark.asyncio
async def test_vaults_performance_history(client_admin_wallet, any_vault):
    with assert_api_calls(client_admin_wallet, expected=1):
        performance = await client_admin_wallet.vaults.performance_history(
...
@pytest.mark.asyncio
async def test_vault_pricing_may_be_unset(any_vault):
...
@pytest.mark.asyncio
async def test_vaults_list_curated(client_admin_wallet):
    with assert_api_calls(client_admin_wallet, expected=1):
        curated = await client_admin_wallet.vaults.list_curated()
    assert isinstance(curated, VaultIdsResponse)


@pytest.mark.asyncio
async def test_vaults_list_shareholdings(client_admin_wallet):
    with assert_api_calls(client_admin_wallet, expected=1):
        held = await client_admin_wallet.vaults.list_shareholdings()
    assert isinstance(held, VaultIdsResponse)


@pytest.mark.asyncio
async def test_vaults_shares(client_admin_wallet):
    with assert_api_calls(client_admin_wallet, expected=1):
        shares = await client_admin_wallet.vaults.shares()
    assert isinstance(shares, VaultSharesResponse)


@pytest.mark.asyncio
async def test_vaults_list_live_requests(client_admin_wallet):
    with assert_api_calls(client_admin_wallet, expected=1):
        live = await client_admin_wallet.vaults.list_live_requests()
...
async def test_vaults_request_history(client_admin_wallet):
    with assert_api_calls(client_admin_wallet, expected=1):
        history = await client_admin_wallet.vaults.request_history(page=1, page_size=10)
...
@pytest.mark.asyncio
async def test_vaults_list_live_mint_requests(client_admin_wallet, curated_vault):
    with assert_api_calls(client_admin_wallet, expected=1):
        queue = await client_admin_wallet.vaults.list_live_mint_requests(vault_subaccount_id=curated_vault)
    assert isinstance(queue, MultipleVaultRequestsResponse)


@pytest.mark.asyncio
async def test_vaults_list_live_burn_requests(client_admin_wallet, curated_vault):
    with assert_api_calls(client_admin_wallet, expected=1):
        queue = await client_admin_wallet.vaults.list_live_burn_requests(vault_subaccount_id=curated_vault)
    assert isinstance(queue, MultipleVaultRequestsResponse)


@pytest.mark.asyncio
async def test_vaults_update_info(client_admin_wallet, curated_vault):
    ...
    vault = await vault_subaccount.vaults.get()
    with assert_api_calls(client_admin_wallet, expected=1):
        ack = await vault_subaccount.vaults.update_info(description=vault.description)
    assert ack.status
...
@pytest.mark.asyncio
async def test_vaults_request_deposit_and_cancel(client_admin_wallet, any_vault):
    ...
    try:
        with assert_api_calls(client_admin_wallet, expected=1):
            ack = await client_admin_wallet.vaults.request_deposit(
        ...
        cancelled = await client_admin_wallet.vaults.cancel_all_requests(vault_subaccount_id=vault_id)
        assert isinstance(cancelled, VaultCancelResponse)


@pytest.mark.asyncio
async def test_queued_deposit_appears_in_the_live_queue(client_admin_wallet, any_vault):
    ...
    try:
        ...
        await client_admin_wallet.vaults.cancel_all_requests(vault_subaccount_id=vault_id)


@pytest.mark.asyncio
async def test_cancel_all_requests_is_idempotent(client_admin_wallet, any_vault):
    ...
    vault_id = any_vault.protocol.subaccount_id
    await client_admin_wallet.vaults.cancel_all_requests(vault_subaccount_id=vault_id)
    again = await client_admin_wallet.vaults.cancel_all_requests(vault_subaccount_id=vault_id)
    assert again.cancelled_request_ids == []


@pytest.mark.asyncio
async def test_vaults_request_withdraw_needs_shares(client_admin_wallet, any_vault):
    ...
    vault_id = any_vault.protocol.subaccount_id
    holdings = await client_admin_wallet.vaults.shares()
    ...
    try:
        ack = await client_admin_wallet.vaults.request_withdraw(
            vault_subaccount_id=vault_id,
...
@pytest.mark.asyncio
async def test_intent_signature_expiry_defaults_well_beyond_an_hour(client_admin_wallet, any_vault):
