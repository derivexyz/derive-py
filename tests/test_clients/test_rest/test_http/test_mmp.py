...
def test_mmp_get_config(client_admin_wallet):
    currency = None
    mmp_configs = client_admin_wallet.mmp.get_config(currency=currency)
    assert isinstance(mmp_configs, list)
    assert all(isinstance(item, MmpConfigResult) for item in mmp_configs)


def test_mmp_set_config(client_admin_wallet):
    currency = "BTC"
    mmp_frozen_time = 1
    mmp_interval = 1
    mmp_amount_limit = 0
    mmp_delta_limit = 0
    set_mmp_config = client_admin_wallet.mmp.set_config(
        currency=currency,
...
def test_mmp_reset(client_admin_wallet):
    currency = "ETH"
    result = client_admin_wallet.mmp.reset(currency=currency)
